...
const EditingDay: React.FunctionComponent<EditingDayProps> = ({ date, onExit }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            window.alert("Time entries uploaded successfully.");
            onExit();
        } catch (err) {
                            ...
                            <tr key={index}>
                                ...
                                <td>
                                    <input type="text" className="border rounded px-2 py-1 w-full"
                                        value={row.qualifier} onChange={(e) => updateQualifier(index, e.target.value)} />
                                </td>
                                <td>
